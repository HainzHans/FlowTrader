...
export class AccountsSection implements OnInit {
  ...
  readonly accountTypes: { value: AccountType; label: string; icon: string; desc: string; color: string }[] = [
  ...
  readonly currencies = ['USD', 'EUR', 'GBP', 'CHF', 'JPY', 'BTC'];
  readonly colors     = ['#22c55e', '#7c3aed', '#3b82f6', '#ef4444', '#f59e0b', '#ec4899', '#14b8a6', '#f97316'];
  ...
  async loadAccounts(): Promise<void> {
    ...
    const { data, error } = await this.accountsService.getAll();

    if (error) {
      // Fallback: lokale Demo-Daten wenn Tabelle noch nicht existiert
      console.warn('[AccountsSection] DB not available, using demo data:', error);
      this.accounts.set(this.getDemoAccounts());
    } else {
  ...
  async save(): Promise<void> {
    ...
    const balance = typeof f.startBalance === 'string'
      ? parseFloat(f.startBalance)
      : f.startBalance;
    ...
    if (editing) {
      const { error } = await this.accountsService.update(editing.id, {
        name:         f.name,
        broker:       f.broker,
        type:         f.type,
        currency:     f.currency,
      ...
      if (userId) {
        const { data, error } = await this.accountsService.create(userId, {
          name:         f.name,
          broker:       f.broker,
          type:         f.type,
          currency:     f.currency,
        ...
        // Fallback: lokal hinzufügen wenn keine DB-Verbindung
  ...
  async deleteAccount(): Promise<void> {
    ...
    const { error } = await this.accountsService.delete(id);
  ...
  accountUpdateColor(color: string):           void { this.accountForm.update(f => ({ ...f, color })); }
  accountUpdateStartBalance(value: number):    void { this.accountForm.update(f => ({ ...f, startBalance: value })); }
  accountUpdateCurrency(currency: string):     void { this.accountForm.update(f => ({ ...f, currency })); }
  accountUpdateType(type: AccountType):        void { this.accountForm.update(f => ({ ...f, type })); }
  accountUpdateBroker(broker: string):         void { this.accountForm.update(f => ({ ...f, broker })); }
  accountUpdateName(name: string):             void { this.accountForm.update(f => ({ ...f, name })); }
